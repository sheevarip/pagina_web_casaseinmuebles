 ...
 $Id_anuncio = ExecuteQuery("INSERT INTO propiedades (Id_usuarios,titulo,titulo_SEO,Id_oferta,Id_inmueble,clave,descripcion,recamaras,banos,superficie_terreno,superficie_construccion,niveles,estacionamientos,precio,municipio,colonia,direccion,disponible_desde,fecha,estado)
	 ...
	 //echo "INSERT INTO propiedades (Id_usuarios,titulo,titulo_SEO,Id_oferta,Id_inmueble,clavebim,descripcion,recamaras,banos,superficie_terreno,superficie_construccion,niveles,estacionamientos,precio,municipio,colonia,direccion,disponible_desde,fecha,estado) VALUES ('".$sesion_usuarios."','".$titulo."','".$titulo_SEO."','".$oferta."','".$Datos_inmuebles['Id']."','".$clave."','".$descripcion."','".$recamara."','".$banos."','".$terrenos."','".$construccion."','".$niveles."','".$estacionamiento."','".$precio."','".$municipio."','".$colonia."','".$direccion."','".$fecha."','".date('Y-m-d H:i:s')."',1)";
		...
		if(is_numeric($Id_anuncio) && $Id_anuncio != 0){
			...
			$carpetaDestino=CarpetaUsuario($Id_anuncio,"../fotos");		 
					...
					if($_FILES["fotos"]["type"][$i]=="image/jpeg" || $_FILES["fotos"]["type"][$i]=="image/pjpeg" || $_FILES["fotos"]["type"][$i]=="image/gif" || $_FILES["fotos"]["type"][$i]=="image/png")
					{
						...
						{*/
							...
							if(@move_uploaded_file($origen, $destino))
							{
								...
								ResizePhoto($carpetaDestino,str_replace('/fotos','/fotos_tooltips',$carpetaDestino),$Fotosname,$Fotosname,'241');	
								 ExecuteQuery("INSERT INTO foto(Id_propiedades,fotos,url_fotos) VALUES ('".$Id_anuncio."','".$destino."','".str_replace('/fotos','/fotos_tooltips',$carpetaDestino).$Fotosname."')");
			...
			if(!empty($Error)){
				//ExecuteQuery("DELETE FROM propiedades WHERE Id = '".$Id_anuncio."'");
			//}else{			
